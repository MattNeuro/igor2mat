...
*/
int
ReadWave(CP_FILE_REF fr, int* typePtr, long* npntsPtr, void** waveDataPtrPtr, char ** name)
{
...
void doMatlabWrite (long npnts, float * data, char * name, MATFILE *mf) 
{
	int i;
	double * buffer;
...
void doMatlabStringWrite (long npnts, char * data, char * name, MATFILE *mf) 
{
	int i;
	char * buffer;

	printf("Found %d points. First: %s", npnts, (char) data[0]);

	buffer = (char *) malloc(sizeof(char) * 10 * (npnts + 1));
	for (i = 0; i < npnts; i++) {
		buffer[i] = (char) data[i];
	}

	matfile_addstring(mf,name, buffer);
}
